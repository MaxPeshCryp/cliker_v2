...
`
    ...
    try {
        ...
        // Voluntary ad: dismissing pays nothing; finishing pays once and survives reload.
        await page.locator('[data-ad-placement="balance"]').click()
        await page.waitForFunction(() => Boolean(adSession))
        assert.match(await page.locator('.ad-placeholder').textContent(), /Здесь могла быть ваша реклама/)
        assert.equal(await page.locator('#adClaim').isDisabled(), true)
        await page.keyboard.press('Escape')
        assert.equal(await page.locator('#adDialog').isVisible(), false)
        assert.equal((await (await context.request.get(`${url}/api/state`)).json()).userCount, 0)
        await page.locator('[data-ad-placement="balance"]').click()
        await page.waitForFunction(() => !document.querySelector('#adClaim').disabled)
        if (process.env.UI_SCREENSHOT_DIR) {
            fs.mkdirSync(process.env.UI_SCREENSHOT_DIR, { recursive: true })
            await page.screenshot({ path: path.join(process.env.UI_SCREENSHOT_DIR, 'ad-desktop.png') })
        }
        await page.locator('#adClaim').click()
        await page.waitForFunction(() => document.querySelector('#adStatus').textContent.includes('Готово!'))
        assert.equal((await (await context.request.get(`${url}/api/state`)).json()).userCount, 100)
        await page.locator('#adClose').click()
        await page.reload()
        await page.locator('[data-ad-card="balance"]').waitFor({ state: 'visible' })
        assert.equal(await page.locator('[data-ad-placement="balance"]').isDisabled(), true)
        ...
        const scoreBeforeAd = await page.locator('#timedResultScore').textContent()
        await page.locator('[data-ad-placement="result"]').click()
        assert.equal(await page.locator('#adDialog').evaluate(node => node.matches(':modal')), true)
        await page.waitForFunction(() => !document.querySelector('#adClaim').disabled)
        await page.locator('#adClaim').click()
        await page.waitForFunction(() => document.querySelector('#adStatus').textContent.includes('Готово!'))
        await page.locator('#adClose').click()
        assert.equal(await page.locator('#timedDialog').isVisible(), true)
        assert.equal(await page.locator('#timedResultScore').textContent(), scoreBeforeAd)
        assert.equal(await page.locator('[data-ad-placement="result"]').isDisabled(), true)
        ...
        await page.locator('[data-endgame-tab="boosts"]').click()
        await page.locator('[data-ad-placement="boost"]').click()
        assert.equal(await page.locator('#adDialog').evaluate(node => node.scrollWidth <= node.clientWidth), true)
        if (process.env.UI_SCREENSHOT_DIR) await page.screenshot({ path: path.join(process.env.UI_SCREENSHOT_DIR, 'ad-mobile.png') })
        await page.waitForFunction(() => !document.querySelector('#adClaim').disabled)
        await page.locator('#adClaim').click()
        await page.waitForFunction(() => document.querySelector('#adStatus').textContent.includes('Готово!'))
        assert.ok((await (await context.request.get(`${url}/api/state`)).json()).boosts.income_x2 > 0)
        await page.locator('#adClose').click()
        ...
        console.log('PASS: voluntary ad cancellation, timed reward claim, saved cooldown, result bonus without ranking changes, boost and mobile ad layout.')
